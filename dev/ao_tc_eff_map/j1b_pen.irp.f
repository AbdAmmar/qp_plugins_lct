...
BEGIN_PROVIDER [ double precision, j1b_gauss_coeff, (nucl_num) ]
  ...
  IRP_IF MPI
    include 'mpif.h'
    integer :: ierr
    call MPI_BCAST(j1b_gauss_coeff, (nucl_num), MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
    if (ierr /= MPI_SUCCESS) then
      stop 'Unable to read j1b_gauss_coeff with MPI'
    endif
  IRP_ENDIF
  ...
  if (exists) then
    ...
    if (mpi_master) then
      write(6,'(A)') '.. >>>>> [ IO READ: j1b_gauss_coeff ] <<<<< ..'
      call ezfio_get_ao_tc_eff_map_j1b_gauss_pen(j1b_gauss_coeff)
      IRP_IF MPI
        call MPI_BCAST(j1b_gauss_coeff, (nucl_num), MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, ierr)
        if (ierr /= MPI_SUCCESS) then
          stop 'Unable to read j1b_gauss_coeff with MPI'
